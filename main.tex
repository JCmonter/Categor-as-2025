...
\input{macros.tex} %Cargar en este archivo los paquetes que requiera el autor, 
...
\author[Vázquez A., Monter J. C., Medina M., Zaldívar L. A.]{Alejandro Vázquez Aceves, Juan Carlos Monter Cortés, Mauricio Medina Barcena y Luis Ángel Zaldívar Corichi}
...
\begin{obs}\label{adjcomp}
	Supongamos que tenemos $F\dashv G$ funtores adjuntos entre $\mathcal{C}$ y $\mathcal{D}$, dadas flechas $(FA\xrightarrow p B)\in\mathcal{D}$ y $(A\xrightarrow q GB)\in\mathcal{C}$ les corresponden flechas \'unicas $(A\xrightarrow{\Bar{p}}GB)$	y $(FA\xrightarrow{\Bar{q}}B)$,
...
\begin{proof}
	...
	y hay que demostrar que cada una de estas componentes es la identidad en $A$. Usando el primer diagrama del Lemma \ref{lemmatriangulo} para el morfismo $\eta_A:A\to FG(A)$, se tiene que $\varepsilon_{FA}\circ F(\eta_A)=\overline{\eta_A}=\overline{\overline{\id_A}}=\id_A$.
...
Esta construcción empieza con una adjunción $F\dash G$ donde ya esta tenemos a la categoría $\EuScript{D}$ que soluciona el problema de factorización, es decir tenemos:
...
propiedades deseadas, por último cabe notar que aunque no se pidió, es claro que $F_{T}$ es un funtor, el problema con el es que no es muy interesante a diferencia de $G_{T}$.
...
tales que definen a $(T,\eta,\mu)$ y las flechas de $\mathrm{Ad}\EuScript{C}$ están dadas como sigue, si \[(F,G,\eta,\epsilon)\colon\EuScript{C}\rightarpoonup \EuScript{A}\]
 ...
 es un par de funtores $K\colon\EuScript{A}\rightarrow\EuScript{A}'$ y $L\colon\EuScript{C}\rightarrow\EuScript{C}$ tal que
