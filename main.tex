...
Si $(F,G,\eta,\epsilon)$ son datos de una adjunción con $F\colon \C\rightarrow\D$ entonces la terna $(GF, \eta,G\epsilon F)$ define una monada en $\C$.
\end{prop}   
...
Vale la pena en este punto (supongo) poner un poco de contexto aún mas abstracti sin-sentido, denotemos por $\mathrm{Ad}\EuScript{C}$ a la categoría de adjunciones para nuestra categoría favorita $\EuScript{C}$
Hemos así probado que :

\begin{prop}\label{kleisli es final}
Dada una categoría $\EuScript{C}$ para una monada de Kleisli $(T,\eta,(\_)^{*})$ ( o terna de Kleisli) se tiene que la categoría de Kleisli de una monada de Kleisli $(T,\eta,(\_)^{*})$ ( o terna de Kleisli) se tiene que la categoría de Kleisli de $\EuScript{C}_{T}$
es un objeto final en la categoría $\mathrm{Ad}\EuScript{C}$, es decir, dada cualquier monada esta da lugar a una adjunción del dominio del adjunto izquierdo, y si tal factorización existe entonces la categoría de Kleisli es el objeto inicial del codominio de tal adjunción.
\end{prop}
