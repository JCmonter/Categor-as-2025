...
Intrudcción sobre el lema de yoneda y enunción y demostración... o solo un sckecth suficiemente convincente de que funciona la prueba

\section{Adjunciones}

\begin{dfn}
	NO HAY NADA DE Adjunciones
\end{dfn}

\subsection[Monadas una pequeña introducción]{Monadas una pequeña introducción}\label{SECMON}


Esta mini sección tiene el objetivop de introducir otra faceta de las adjunciones, empecemos con una situación de abstracta sin sentido.


Dado un funtor $T\colon \EuScript{C}\rightarrow\EuScript{C}$ qusieramos (por que no) factorizarlo es decir, queremos encontrar una categoría $\D$ y funtores $F\colon \C\rightarrow\D$,
y $G\colon \D\rightarrow \C$ tal que: \[T=GF\;\;\;\; F\dashv G.\]

Esta factorización nos da una \emph{presentación universal de } $T$ donde universal querra decir \emph{libre}
...
\end{enumerate}
Fabricamos el siguiente funtor, $(\_)_{\perp}\colon\D\C\Pos\rightarrow \D\C\Pos $ donde el "elemento" $\perp$ esta fijo, entonces este funtor manda  a ncada copo directamente completo digamos,

$D$ a este mismo solo que se adjunta $\perp$ como elemento menor, la acción en flechas es la obvia, $f\colom A\rightarrow B$ entonces \[\begin{cases} f_{\perp}(a)
    a & a\in D\hookrightarrow D_{perp} \\
    \perp & a=\perp
  \end{cases}\]
  \item[(4)]
  \begin{enumerate}
 \item[(a)] Consideremos una de nuestras categorías favoritas, $Top$ la categoría de espacios topologicos junto con funciones continuas,
fijemos nuestra atención en la subcategoría reflexiva de espacios topologicos $T_{0}$, denotemos por $Top_{0}$ a esta subcategoría plena, 
...
\item[(b)]
En esta misma digamos, vertiente denotemos por $\EuScript{S}Top$ la categoría de espacios sobrios localmente compactos junto con funciones continuas como nuestars flechas,
...
de manera analoga a la anterior tenemos un funtor $(\_)^{\square}\colon \EuScript{S}Top\rightarrow \EuScript{S}Top$ solo que en flechas se calcula como \[f^{\square}(X)=f[X]\]
  \end{enumerate}
\end{itemize}
  \section[Mon]{Monadas} 
De acuerdo con nuestro probelams sin sentido, es decir, la factorización que queremos construir para $T\colon\C\rightarrow\C$, necesitamos que exista una adjunción $\D$ y funtores $F\colon \C\rightarrow\D$,
y $G\colon \D\rightarrow \C$ tal que: \[T=GF\;\;\;\; F\dashv G.\], entonces tenemos que asegurar que existan
  flechas \[\eta\colon id\_{\C}\rightarrow GF\text{ y }\;\; \epsilon\colon FG\rightarrow id_{\D}\] de tal manera que para todo $A\in\C$ y para todo $D\in\D$ los triangulos:
  \[
    ...
    \begin{tikzcd}[column sep=large, row sep=large]
        ...
        & FA 
      \end{tikzcd}
y así podemos exigir la existenbcia de una transformación natural \[\mu\colon T^{2}\rightarrow T\]

Entonces para tener una definicion adecuada de lo que es la factorización necesitamos tener una terna $(T,\eta,\mu)$. Es instructivo para el lector (neurotico) calcular $\eta$ y $\mu$ de nuestrso ejemplos clave.

Sin embargo necesitamos aun mas información para tener una definicion adecuada de lo que sera nuestra monada.
...
\[\begin{tikzcd}[column sep=large, row sep=large]
  ...
  \end{tikzcd}
\]
  Y si en elk triangulo para $G$ tomamos la componente con $D=FA$ uno obtiene:

\[\begin{tikzcd}[column sep=large, row sep=large]
   TA=GFA \arrow[r, "\eta_{TA}=\eta GFA"] \arrow[dr, "1_GFA"'] & T^{2}A=GFGFA \arrow[d, "\mu_{A}=G\varepsilon_{FA}"] \\
    & TA=GFA
  \end{tikzcd}\]
es decir, \[id_{TA}=\mu_{A}T\eta_{A}\text{ y } id_{TA}=\mu_{A}\eta_{TA}.\] es decir tenemos un tipon de \emph{conmutatividad} entre $T\eta_{A}$ y $\eta_{TA}$ y estas identidades
son las mas cercanas a nuestro caso original (recuerden que empezamos con una factorizacion chida).
...
Resulta (o deberia de ser) que esta definición es suficiente para tener la factorización dada, para realemnte justificar esto necesitamos un poco mas de trabajo.

\section{Ternas de Kleisli}\label{KLEI}

La definición de monada que dimos \ref{mona} se derivo de la defnición de adjunción dada por las identidad triangulares, pero como sabemos existen tres formas clásicas de definir adjunción veamos que sucede con la dada por flechas universales.
requiere la existencia de un funtor $G\colon\D\rightarrow \C$ y una asiganción (no se requiere funtorialidad) $F$ que solo actue en objetos de $\C$ a objetos de $\D$, pero esto esta subyugado al hecho de que para cada objeto $A\in\C$ y cada flecha
$\eta_{A}\colon A\rightarrow GFA$ tal que para todp \[f\colon A\rightarrow GD\] con $D$ cualquier obejto de $\D$ exista una única flecha \[f^{+}\colon FA\rightarrow D\] de tal manera que \[f=Gf^{+}\eta_{A}.\], es decir tenemos
una función \[(\_)^{+}\colon\C(A,GD)\rightarrow\D(FA,D).\] en un dibujo:

\[\begin{tikzcd}[column sep=large, row sep=large]
    A \arrow[r, "\eta_{A}"] \arrow[dr, "\forall f"'] & GFA \arrow[d, dashed, "Gf^{+}"] \\
    & GD
  \end{tikzcd}\]
Esto en $\C$

  \[\begin{tikzcd}[column sep=large, row sep=large]
    FA \arrow[d, dashed, "\exists !f^{+}"] \\
    & D
  \end{tikzcd}\]
Esto otro en $\D$.

Como antes queremos dado $T$ encontrar $F$ y $G$ de tal manera que hagan la gracia de arriba y factoricen a $T$, la ventaja de esta defnición es que no siendo $F$ un funtor a priori, podemos entonces empezar $T$ mandando objetos en objetos.
...
, y así se debe de pedir, que para cada objeto $A$ en $\C$ exista una flecha $\eta_{A}\colon A\rightarrow TA$. En este caso el operador de flechas $(\_)^{+}$ esta descrito como sigue: Si \[f\colon A\rightarrow GFB\] entonces
 ...
 \[\C(A,TB)\rightarrow\C(TA,TB)\] ahora desglocemos las ecuaciones que ligan la receta de $(T,\eta,(\_)^{*})$.
 Imitando un poco la universalidad de la flecha que nos da la defnición de adjunción tenemos que pedir:
 \[f^{*}\eta_{A}=f.\] 
 Y entonces si $\eta_{A}\colon A\rightarrow GFA$ y por la propiuedad universal debemos tener :\[Gid_{FA}\circ\eta_{A}=\eta_{A}=G\eta^{+}_{A}\circ\eta_{A}\]
...
Que por unicidad debemos de tener \[(Gg^{+}\circ f)^{+}=g^{+}\circ f^{+}\] y así \[G(Gg^{+}\circ f)^{+}=Gg^{+}\circ Gf^{+}.\] Entonces requieremos para nuestro operador $(\_)^{*}$ lo analogo, es decir, \[(g^{*}\circ f)^{*}=g^{*}\circ f^{*}\]
...
\end{dfn}
Y claro tenemos la proposición correspondiente con la definición de adjuncion mediante flechas universales.
...
\end{prop}    
El lector perspicaz notara que debe de haber una relación entre la defnición \ref{mona} y la defnición \ref{Kleislitriple}, pues lo esperado sucede:
...
y sea su correspondiente $Tf\colon Ta\rightarrow TB$ dado: \[Tf=(\eta_{B}\circ f)^{*}\] esto lo hace funtorial ya que poara todo $A\in\EuScript{C}$ se tiene: 
\[Tid_{A}=(\eta_{A}\circ id_{A})^{*}=\eta_{A}^{*}=id_{TA}.\]

y para felchas \[\begin{tikzcd}[column sep=large, row sep=large]
    A \arrow[r, "f"]  & B \arrow[r, "g"] & C
    \end{tikzcd}\]
...
Y así $(T,\eta,\mu)$ define uan monada en $\EuScript{C}$.

Recíprocamente, si comentzamos con los datos de una monada $(T,\eta,\mu)$ en $\EuScript{C}$., claro casi tautologicamente tenemos que $T$ y $\eta$ satisfacen las condiciones de la definición
...
como se pedia.
Ahora para $f$ como antes tenemois con $g\in\EuScript{C}(A,TB)$, entonces calculamos usando la funtorialidad de $T$: 
...
$(T,\eta,(\_)^{*})\leadsto (T,\eta,\mu)$ y vicersa forman una biyección, en efecto si empezamos con una de Kleisli $(T,\eta,(\_)^{*})\leadsto (T,\eta,\mu)$ y su monada construida como antes,  entonces para cualquier flecha $f\colon A\rightarrow TB$ y entonces \[\mu_{B}\circ Tf=id^{*}_{TA}\circ(\eta_{B}\circ f)^{*}=(id^{*}_{TA}\circ\eta_{B}\circ f)^{*}=(id_{TA}\circ f)^{*}=f^{*}.\]
y así obtenemos la monada de Kleisli original. 
Y vicersa si empezamos con una monada usual $(T,\eta,\mu)$ con su Kleisli y de alli obtenemos la monada dada por esta entonces con los calculos que tenemos hechos \[id^{*}_{TA}=\mu_{TA}\circ Tid_{TA}=\mu_{TA}\circ id_{T^{2}A}=\mu_{TA}.\]
y así recuperamos la monada con la que empezamos.

\end{proof}   


Esto esta bien, ya tener dos opciones para cocinar la factorización que estamos busncando, pero aún así necesitamos una solución \emph{universal}.
...
La solución que describeremos aquí es debida a Kleisli en su celebra artículo Every standard construction is induced by a pair of adjoint functors (hayu que ponerlo enla biblio)
Esta construcción empieza con una adjunción $F\dash G$ donde ya esta tenemos a la categoría $\EuScript{D}$ que soluciona el problema de factorizaicón, es decir tenemos:
...
\item[(ii)] Las flechas $A\rightarrow B$ estan dadas por $A\rightarrow TB$;
\item[(iii)] Las identidades para cada $A$ estan dadas por $\eta_{A}$.
\item[(iv)] La composición esta dada, $f\colon A\rightarrow B$  y $g\colon B\rightarrow C$ en $\EuScript{C}_{T}$ : \[\begin{tikzcd}[column sep=large, row sep=large]
    ...
    \end{tikzcd}\]
\end{itemize}
Formalmente deberiamos dintinguir entre las composicion en $\EuScript{C}$ y la de $\EuScript{C}_{T}$ para esto digamos que el simbolo $g\bullet f$ representa la composición en $\EuScript{C}_{T}$.

También nos falta verificar que en efecto los datos de \ref{SOLKLEI} constituyen uan categoría, para esto si $f\colon A\rightarrow B$ en $\EuScript{C}_{T}$ tenemos que la composición con la identidad esta dada como \[f^{*}\circ\eta_{A}=f.\]
...
Con la notación de la defnición \ref{SOLKLEI} se tiene que $\EuScript{C}_{T}$ es una categoría.
\end{prop}

Existe una comparasión directa entre objetos de $\EuScript{C}$ y $\EuScript{C}_{T}$, $F_{T}\colon Obj\EuScript{C}\rightarrow Obj\EuScript{C}_{T}$ esta asignación no hace nada perse, también podemos comparar con un funtor
...
$G_{T}f^{+}=f^{*}$ y calculando \[G_{T}f^{+}\circ\eta_{A}=f^{*}\circ\eta_{A}=f\] como se requeria. Ahora para la unicidad, si $g\colon F_{T}A\rightarrow B$ en $\EuScript{C}_{T}$
dado por una flecha $g\colon A\rightarrow TB$ en $\EuScript{C}$ entonces \[G_{Tg}\circ\eta_{A}=g^{*}\circ\eta_{A}=g,\] entonces tenemos que $g=f$, por lo tanto hemos encontrado una adjunción con las
propiedades deseadas, por último cabe notar que aunque no se pidio, es claro que $F_{T}$ es un funtor, el problema con el es que no es muy interesante a diferencia de $G_{T}$.
...
Vale la pena en este punto (supongo) poner un poco de contexto aún mas abstracto sin-sentido, sea $(T,\eta,\mu)$ una monada en una categoría $\EuScript{C}$.
denotemos por $\mathrm{Ad}\EuScript{C}$ a la categoría de todas las adjunciones $(F,G,\eta,\epsilon)\colon\EuScript{C}\rightharpoonup \EuScript{A}$
tales que definen a $(T,\eta,\mu)$ y las felchas de $\mathrm{Ad}\EuScript{C}$ estan dadas como sigue, si \[(F,G,\eta,\epsilon)\colon\EuScript{C}\rightarpoonup \EuScript{A}\]
 y \[(F',G',\eta',\epsilon')\colon\EuScript{C}\rightarpoonup \EuScript{A}'\] son adjunciones sobre $\EuScript{C}$, un morfismo entre estas \[(F,G,\eta,\epsilon)\rightarrow(F',G',\eta',\epsilon')\] 
 es un par de funtores $K\colon\EuScript{A}\rightarrow\EuScript{A}'$ y $L\colon\EuScript{C}\rightaarow\EuScript{C}$ tal que
...
\begin{thm}(Comparasión de Kleisli)\label{kleisli es final}
Dada una adjunción $(F,G,\eta,\epsilon)$ en una categoría $\EuScript{C}$, y sea $T=(GF, \eta,G\epsilon F)$ la monada que esta define,
entonces existe un único funtor $L\colon\EuScript{C}_{T}\rightarrow\EuScript{A}$ con $GL=G_{T}$ y $LF_{T}=F$.
\end{thm}


\begin{thm}\label{kleisli es final}
Dada una monada $(T,\eta,\mu)$ en una categoría $\EuScript{C}$, 
\end{thm}


En la práctica ya que estamos hablando de objetos en la categoría de adjunciones de una categoría agradable, podriamos preguntarnos si tal categoría tiene objeto final.
...
Este capítulo esta dedica a desglosar varias cosas que hemos visto en un ejemplo que es de sumo interes.
...
Ahora construiremos un funtor de regreso $\pt\colon\Frm\to\Top$
